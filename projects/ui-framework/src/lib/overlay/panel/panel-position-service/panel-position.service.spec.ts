...
import {
  CdkOverlayOrigin, ConnectedOverlayPositionChange, Overlay, PositionStrategy, ScrollStrategy, ScrollStrategyOptions,
...
describe('PanelPositionService', () => {
  ...
  beforeEach(() => {
    ...
    scrollStrategyOptions = createSpyObj('scrollStrategyOptions', ['reposition']);
  ...
  describe('getPanelPositionStrategy', () => {
    it('should return strategy where above has priority', () => {
      const panelDefaultPosVer: PanelDefaultPosVer = PanelDefaultPosVer.above;
      const strategy: PositionStrategy = panelPositionService
        .getPanelPositionStrategy(cdkOverlayOriginMock, panelDefaultPosVer);
    ...
    it('should return strategy where below has priority', () => {
      const panelDefaultPosVer: PanelDefaultPosVer = PanelDefaultPosVer.below;
      const strategy: PositionStrategy = panelPositionService
        .getPanelPositionStrategy(cdkOverlayOriginMock, panelDefaultPosVer);
      expect(strategy['positions']).toEqual([
        BELOW_CENTER,
  ...
  describe('getCenterPanelPositionStrategy', () => {
    it('should return strategy with center position bottom, center position top', () => {
      const strategy: PositionStrategy = panelPositionService
        .getCenterPanelPositionStrategy(cdkOverlayOriginMock);
      expect(strategy['positions']).toEqual([
        BELOW_CENTER,
        ABOVE_CENTER,
      ]);
