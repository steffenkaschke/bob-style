...
const note = `
  ...
  getPaletteColors<wbr>(count, colorSet?: PalletteColorSet) | ColorPalette[] | returns Count number of colors from set (by default, from 'main'), in correct order. <br>If count is falsey, will return all colors in set.
  getRandomPaletteColors<wbr>(count, colorSet?: PalletteColorSet) | ColorPalette[] | returns Count number of colors (by default from 'main' set); <br>pass null for count to get all set colors, randimized.
